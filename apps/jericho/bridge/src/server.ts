...
function openVoiceSession(webSocket: WebSocket, options: JerichoServerOptions, tools: ToolExecutor): void {
  ...
  const confirmedJerichoVoice = resolvePresentationVoice({
    preferencePath: options.voicePreferencePath,
    configuredVoice: options.geminiVoice,
  });
  let confirmedVoice = confirmedJerichoVoice;
  const personas = createPersonas({
    jerichoVoice: confirmedJerichoVoice,
  ...
  const confirmVoice = (voice: string) => {
    ...
    personas.jericho.voice = preference.voice;
    currentMode = 'jericho';
    send({
  ...
  webSocket.on('message', (raw) => {
    try {
      ...
      if (message.type === 'set_mode') {
        const requestedMode = migratePersonaMode(message.mode);
