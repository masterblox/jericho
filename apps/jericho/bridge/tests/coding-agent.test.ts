...
describe('bounded coding-agent lifecycle', () => {
  ...
  it('selects a project, creates an economical explicit-model session, and verifies a committed SHA', async () => {
    ...
    expect(conductor.createInput).toMatchObject({ projectId: 'project-1', agent: 'codex', model: 'gpt-5-mini' });
...
describe('Conductor CLI adapter', () => {
  it('uses JSON CLI calls and never places a token in argv', async () => {
    const calls: string[][] = [];
    const runner = vi.fn(async (_executable: string, args: readonly string[]) => {
      ...
      if (command === 'projects list') return { stdout: JSON.stringify({ data: [{ id: 'project-1', name: 'Jericho' }] }), stderr: '', exitCode: 0 };
      return { stdout: JSON.stringify({ id: 'ws-1', name: 'task', sessionId: 's-1', deepLink: 'conductor://workspace/ws-1' }), stderr: '', exitCode: 0 };
    });
    const cli = new ConductorCliPort({ runner });
    await expect(cli.authProbe()).resolves.toEqual({ authenticated: true });
    await expect(cli.listProjects()).resolves.toEqual([{ id: 'project-1', name: 'Jericho', repoUrl: undefined }]);
...
function taskBase(): CodingAgentTask {
  return {
    taskId: 'task-1', repositoryPath: '/repo', project: { id: 'project-1' }, workspaceName: 'bounded-task', model: 'gpt-5-mini', prompt: 'Fix the bug.', timeoutMs: 100, pollIntervalMs: 1,
  };
}
