...
export class ConductorConnector implements CaptureConnector {
  ...
  async capture(request: ConnectorCaptureRequest): Promise<ConnectorCapturePage> {
    ...
    const worktrees = findWorktrees(root, root, 0, this.#maxDepth);
    const captures = worktrees.slice(0, request.limit).map((path) => worktreeCapture(configured.id, root, path));
    return {
      captures, failures: [],
      progress: {
      ...
      hasMore: worktrees.length > captures.length,
    };
  }
}
...
function worktreeCapture(rootId: string, root: string, path: string): NormalizedCapture {
